...
interface SpreadMedicineState {
    fishPondWarehouseId: number,
    feedDate: Date,
    docketDetails: StockIssueDocketDetailModel[],
    fishPonds: IdNameModel[],
}
export class SpreadMedicine extends React.Component<RouteComponentProps<{}>, SpreadMedicineState> {
    ...
    async onCuringLivestock() {
        ...
        if (!state.docketDetails || state.docketDetails.length == 0) {
            this.context.ShowGlobalMessage('error', 'Chưa chọn thuốc');
            return false;
        }
        let err = state.docketDetails.find(d => !d.quantity || d.quantity <= 0);
        if (err) {
            this.context.ShowGlobalMessage('error', 'Số lượng thuốc không đúng');
            return false;
        }
        let model = {
        ...
        let response = await LivestockProceedAPICaller.Cure(model);
            ...
            <div className="panel panel-info">
                <div className="panel-body">
                    <div className="col-sm-12">
                        <ProductSimpleSearch
                            productGroupId={SystemIDEnum.ProductGroup_Medicine}
            ...
            <div className="content-wapper">
                <nav aria-label="breadcrumb">
                    <ol className="breadcrumb">
                        ...
                        <li className="breadcrumb-item active" aria-current="page">Rải thuốc</li>
                    </ol>
                </nav>
                <div>
                    {this.renderTabInfo()}
                    {this.renderCustomer()}
                    <div className='col-sm-12'>
                        <button className="btn btn-primary pull-right" onClick={() => this.onCuringLivestock()}>Hoàn tất</button>
                    </div>
                </div>
            </div>
        );
...
interface SpreadMedicineProductProps {
    model: StockIssueDocketDetailModel,
    onChange: Function,
    onDelete: Function,
}
class SpreadMedicineProduct extends React.Component<SpreadMedicineProductProps, any> {
    constructor(props: any) {
        super(props);
    }
    render() {
