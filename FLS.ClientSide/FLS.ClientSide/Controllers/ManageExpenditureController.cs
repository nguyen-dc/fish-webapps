...
namespace FLS.ClientSide.Controllers
{
    ...
    public class ManageExpenditureController : BaseController
    {
        ...
        {
            ResponseConsult<PagedList<ExpenditureDocketModel>> response = await PostAsJsonAsync<PagedList<ExpenditureDocketModel>>(URI_API.EXPENDITURE_TYPE_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _expenditureId)
        {
            ...
            ResponseConsult<ExpenditureDocketModel> response = await GetAsync<ExpenditureDocketModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
