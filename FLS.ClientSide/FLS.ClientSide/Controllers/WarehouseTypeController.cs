...
namespace FLS.ClientSide.Controllers
{
    ...
    public class WarehouseTypeController : BaseController
    {
        ...
        {
            APIResponse<PagedList<WarehouseTypeModel>> response = await PostAsJsonAsync<PagedList<WarehouseTypeModel>>(URI_API.WAREHOUSE_TYPE_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _warehouseId)
        {
            ...
            APIResponse<WarehouseTypeModel> response = await GetAsync<WarehouseTypeModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
