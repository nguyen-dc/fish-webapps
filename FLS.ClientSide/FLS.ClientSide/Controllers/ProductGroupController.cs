...
namespace FLS.ClientSide.Controllers
{
    ...
    public class ProductGroupController : BaseController
    {
        ...
        public async Task<ActionResult> Details(int _groupId)
        {
            ...
            ResponseConsult<ProductGroupModel> response = await GetAsync<ProductGroupModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
        ...
        public async Task<ActionResult> SubgroupList(int _groupId, [FromBody]PageFilterModel _model)
        {
            ...
            ResponseConsult<PagedList<ProductSubgroupModel>> response = await PostAsJsonAsync<PagedList<ProductSubgroupModel>>(link, _model);
            return Ok(response);
        }
