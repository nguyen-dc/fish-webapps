...
namespace FLS.ClientSide.Controllers
{
    ...
    public class ProductUnitController : BaseController
    {
        ...
        {
            APIResponse<PagedList<ProductUnitModel>> response = await PostAsJsonAsync<PagedList<ProductUnitModel>>(URI_API.PRODUCT_UNIT_SEARCH, _model);
            return Ok(response);
        }
        [HttpGet("{_unitId:int}")]
        public async Task<ActionResult> Details(int _unitId)
        {
            ...
            APIResponse<ProductUnitModel> response = await GetAsync<ProductUnitModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
