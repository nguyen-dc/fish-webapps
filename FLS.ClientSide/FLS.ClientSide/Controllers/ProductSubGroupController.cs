...
namespace FLS.ClientSide.Controllers
{
    ...
    public class ProductSubGroupController : BaseController
    {
        ...
        {
            ResponseConsult<PagedList<ProductSubgroupModel>> response = await PostAsJsonAsync<PagedList<ProductSubgroupModel>>(URI_API.PRODUCT_SUBGROUP_SEARCH_ALL, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _subgroupId)
        {
            ...
            ResponseConsult<ProductSubgroupModel> response = await GetAsync<ProductSubgroupModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
