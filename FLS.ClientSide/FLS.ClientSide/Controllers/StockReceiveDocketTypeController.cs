...
namespace FLS.ClientSide.Controllers
{
    ...
    public class StockReceiveDocketTypeController : BaseController
    {
        ...
        {
            APIResponse<PagedList<StockReceiveDocketTypeModel>> response = await PostAsJsonAsync<PagedList<StockReceiveDocketTypeModel>>(URI_API.STOCK_RECEIVE_DOCKET_TYPE_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _stockReceiveDocketId)
        {
            ...
            APIResponse<StockReceiveDocketTypeModel> response = await GetAsync<StockReceiveDocketTypeModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
