...
namespace FLS.ClientSide.Controllers
{
    ...
    public class FarmRegionController : BaseController
    {
        ...
        {
            ResponseConsult<PagedList<FarmRegionModel>> response = await PostAsJsonAsync<PagedList<FarmRegionModel>>(URI_API.FARM_REGION_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _farmRegionId)
        {
            ...
            ResponseConsult<FarmRegionModel> response = await GetAsync<FarmRegionModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
