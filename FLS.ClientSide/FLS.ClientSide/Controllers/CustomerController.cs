...
namespace FLS.ClientSide.Controllers
{
    ...
    public class CustomerController : BaseController
    {
        ...
        {
            APIResponse<PagedList<CustomerModel>> response = await PostAsJsonAsync<PagedList<CustomerModel>>(URI_API.CUSTOMER_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _customerId)
        {
            ...
            APIResponse<CustomerModel> response = await GetAsync<CustomerModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
