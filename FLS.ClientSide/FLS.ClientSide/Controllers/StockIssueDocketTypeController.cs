...
namespace FLS.ClientSide.Controllers
{
    ...
    public class StockIssueDocketTypeController : BaseController
    {
        ...
        {
            ResponseConsult<PagedList<StockIssueDocketTypeModel>> response = await PostAsJsonAsync<PagedList<StockIssueDocketTypeModel>>(URI_API.STOCK_ISSUE_DOCKET_TYPE_SEARCH, _model);
            return Ok(response);
        }
        ...
        public async Task<ActionResult> Details(int _stockIssueDocketId)
        {
            ...
            ResponseConsult<StockIssueDocketTypeModel> response = await GetAsync<StockIssueDocketTypeModel>(link);
            return Ok(response);
        }
        [HttpPost("add")]
